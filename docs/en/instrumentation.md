...
Turning traces or metrics off only stops recording. Context still travels: an incoming
`traceparent` and baggage reach the services you call, and components that still record spans
get the right parent. This holds for `traces.enabled: false`, `metrics.enabled: false`, and
`traces: false` on a single component.

To stop a component from touching requests at all, set both of its own signals to `false`. The
HTTP client and Messenger then add no propagation headers either.
...
If a caller sends `tracestate` or `baggage` as several header lines, the lines are combined in
order. If a request has two `traceparent` headers, the bundle cannot tell which parent is right,
so it starts a new trace. Baggage from that request is still used.

With a `StreamedResponse`, the server span stays open until the body has been sent, but it is not
the current span while your stream callback runs. Queries, HTTP calls and log lines made inside
the callback therefore do not appear under the request's trace. If you need them there, wrap the
work in an operation of its own.
...
`query_text` controls `db.query.text`:

- `sanitized` (default) records the statement with every literal replaced by `?` and comments
  removed, for example `SELECT id FROM users WHERE name = ? LIMIT ?`. A bulk insert is recorded
  with a single row, `VALUES (?, ?)`, however many rows it sends. This is the form the
  OpenTelemetry database conventions allow by default.
- `raw` records the statement exactly as sent, literals included. Literals often hold personal
  data, so use it only when you know your SQL does not.
- `off` records nothing.

The old values still work: `true` means `raw`, and `false` means `off`.

The span name, `db.query.summary`, never contains literals or comments, whatever `query_text` is
set to. It lists the operations and the tables they touch: `SELECT orders`, `INSERT events`,
`CREATE TABLE orders`.

To stay safe, the bundle only reads SQL syntax that every database interprets the same way:
`'...'` strings, quoted names, `-- ` comments and `/* */` comments. If a statement uses anything
else where a string or a comment could begin — a backslash inside quotes, `#`, `$$`, a nested
comment, `--` without a space after it — the bundle does not guess. The span is named after the
database system, such as `mysql`, and gets no sanitized `db.query.text`. SQL generated by the
Doctrine ORM and DBAL does not use any of these.

On MySQL, `"..."` is treated as a string, because it is one unless the session runs with
`ANSI_QUOTES`. DBAL quotes MySQL names with backticks, so generated SQL is not affected.
...
Retries and moves to the failure transport are sent by Messenger itself, not through the
instrumented sender. The message keeps its trace, so the consumer still joins the original
trace, but these resends get no send span and are not counted as sent messages.
...
Spans are exported when a request, a message or a command ends. A long-running command of your
own that loops inside one command therefore exports nothing until it exits. If the span queue
fills up first, new spans are dropped. There is no public API yet to flush in
the middle of a command, so set `OTEL_BSP_MAX_QUEUE_SIZE` large enough for one run, or split the
work into separate commands or Messenger messages.
...
`getItems()` stays lazy, like the pool it wraps. Many adapters only read from the backend while
you iterate over the result, so the `cache.getItems` span ends when the iteration ends, and it
records an error if the backend fails partway through.

While the backend produces an item, the cache span is the current span, so backend work — for
example the query a database-backed pool runs — appears under it. While your loop body runs, it
is not, and that time is not counted in `cache.operation.duration`.

If the result is not read to the end, no duration is recorded:

- if you break out of the loop, the span ends with what was read;
- if you drop the result without reading it, nothing is recorded;
- if the result is still unread when the worker resets between requests, the span ends then.
...
When both are on, each exported record is linked to the trace it was written in. This also works
behind a `buffer` or `fingers_crossed` handler, which sends records later, after the operation
has finished. A record written outside any trace is exported without one, even if the buffer is
flushed during another request. With correlation off, the OpenTelemetry SDK links each record to
whatever trace is current when it is exported, which is only correct when records are not
buffered.
...
| Key | Records | Default |
|---|---|---|
| `doctrine.query_text: raw` | `db.query.text` with literals as sent; the default, `sanitized`, replaces them with `?` | `sanitized` |
