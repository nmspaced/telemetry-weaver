...
final class TelemetryFlusherTest extends TestCase
{
    ...
    private function signal(
        ...
        return new SignalFlusher($provider, FlushPolicy::every($name, 1, $clock), $reporter, 100, $clock);
    }
    ...
    public function allSignalsShareOneDeadlineAndSkippedSignalsRemainEligible(): void
    {
        ...
        $flusher->atBoundary();
        self::assertSame(['traces'], $calls, 'traces spent the whole budget, so the other two were refused');
        self::assertFalse($budget->active());
        self::assertSame(1, $reporter->total());

        $flusher->atBoundary();
        self::assertSame(['traces', 'traces', 'logs', 'metrics'], $calls, 'a refused signal stays eligible');
    }
    ...
    public function nestedFlushDoesNotReplaceTheDeadline(): void
    {
        ...
        $signal = new SignalFlusher($provider, FlushPolicy::onSdkSchedule('traces'), $reporter);
